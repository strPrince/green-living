...
router.get('/home', async (req, res) => {
  try {
    const data = await Echo.find({}).limit(6);

    const events = await Events.find({}).limit(3);
    const items = await Item.find({}).limit(4); // Fetch item data

    if (data.length > 0 || events.length > 0 || items.length > 0) {
